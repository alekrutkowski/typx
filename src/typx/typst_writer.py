...
class TypstWriter:
    ...
    def _materialize_assets(self) -> None:
        ...
        for resource_id, resource in self.doc.resources.items():
            path: Path | None = None
            if resource.source_path and Path(resource.source_path).exists():
                path = Path(resource.source_path)
            elif resource.data is not None and assets_dir is not None:
                assets_dir.mkdir(parents=True, exist_ok=True)
                path = assets_dir / resource.filename
                if path.exists() and path.read_bytes() != resource.data:
                    path = assets_dir / f"{path.stem}-{resource.id[-8:]}{path.suffix}"
    ...
    def _inline(self, inline: Inline) -> str:
        ...
        if isinstance(inline, Bookmark):
            return "" if inline.end else f" <{self._safe_label(inline.name)}>"
        ...
        if isinstance(inline, Citation):
            keys = ", ".join(quote_typst_string(key) for key in inline.keys)
            args = [keys]
    ...
    def _text_style_args(self, style: TextStyle, exclude_simple: bool = False) -> list[str]:
        args: list[str] = []
        if style.font:
            args.append(f"font: {quote_typst_string(style.font)}")
        if style.size_pt:
    ...
    def _par_style_args(self, style: ParagraphStyle) -> list[str]:
        ...
        if style.line_spacing:
            if style.line_spacing_rule == "auto":
                # Typst leading is inter-line space, not Word's line-height multiplier; this is approximate.
                args.append(f"leading: {max(0.0, (style.line_spacing - 1.0) * 11.0):g}pt")
            else:
    ...
    def _native_field(self, field: Field) -> str | None:
        ...
        if match:
            return "@" + self._safe_label(match.group(2))
        ...
        if match:
            return f"#cite({quote_typst_string(match.group(1))})"
