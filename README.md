...
For Typst-to-DOCX conversion, `typx` 0.3.2 makes an unstylized Word document look much closer to Typst 0.15.1 instead of inheriting Microsoft Word's visual defaults. The generated Word styles use Typst's default **Libertinus Serif** body face at 11 pt, compact black heading scales, A4 with 2.5 cm margins, plain links, **DejaVu Sans Mono** raw/code text at 0.8 em, Typst-like quote/figure spacing, zero-margin list markers, and distinct Typst defaults for `table` and `grid`.

Explicit Typst rules still win. For example, `#set text(font: "Arial", size: 14pt)` overrides the body font and size and proportionally scales default headings, while explicit page margins and column gutters override the profile.

The DOCX files generated by the Typst-to-DOCX direction do not themselves embed font binaries. Their OOXML requests **Libertinus Serif** and **DejaVu Sans Mono**, and the generated font table supplies stock-Windows fallbacks: **Palatino Linotype** for the serif face and **Consolas** for the monospace face. This keeps body text serif and code monospace when Typst's bundled fonts are not installed, although exact glyph shapes and line breaks can still differ.

### Fonts when converting DOCX to Typst

For DOCX-to-Typst conversion, `typx` 0.3.2 treats fonts as project assets. It scans the font families actually used by document content and resolves them in this order:

1. font programs embedded in the DOCX font table;
2. matching installed fonts on the conversion machine, including Word `altName` aliases;
3. a warning if neither source is available.

Resolved `.ttf`, `.otf`, or `.ttc` files are copied into the conversion's `<stem>_assets/fonts/` directory. Embedded OOXML fonts are deobfuscated before they are written. The generated Typst uses the corresponding **font family names**, because Typst's `text(font: ...)` parameter selects families rather than font file paths. Typst.app automatically discovers font files uploaded with the project. For local CLI compilation, point Typst at the extracted directory, for example:

```bash
typst compile --font-path report_assets/fonts report.typ
```

The generated `.typ` contains the applicable `--font-path` hint whenever fonts were materialized. Font files are copied only during a user's conversion and are **not** bundled in typx release archives or test corpora. Whether a copied font may be redistributed is governed by that font's own license.

### Word bibliography when converting DOCX to Typst

`typx` 0.3.2 understands Word's bibliography source store and the `CITATION`/`BIBLIOGRAPHY` fields used by desktop Word. Current-document sources stored under `customXml` are converted to a local `bibliography/references.bib` asset. Ordinary single-source citations become native Typst `#cite(...)` calls, Word page locators become citation supplements, and a Word bibliography field becomes `#bibliography(...)`. Common Word bibliography styles such as APA, IEEE, MLA, and Chicago author-date are mapped to Typst's built-in styles.

Word can also cache citation displays or group sources in ways that do not map one-to-one to Typst. For those grouped or unsupported cases, `typx` preserves Word's cached visible citation while adding non-displaying Typst citations so the referenced works still appear in the generated bibliography.
...
--update-fields  # opt in to recalculating Word fields on open
...
- headings, bookmarks, links, fields, numbered cross-references, page references, and citations;
- automatic PAGE/NUMPAGES numbering in headers or footers;
- ordered, unordered, nested, restarted, and custom-format lists with Typst-like top-level marker alignment;
...
Visual pagination is also renderer-dependent. Typst and Word use different shaping, line-breaking, table-layout, floating-object, and pagination engines. Equivalent semantics do not guarantee identical page coordinates. Static heading, figure, equation, and page references in the supported subset are lowered to Word hyperlinks/fields, but arbitrary counter functions and custom reference show rules still require Typst evaluation.
